...
export default function ProjectsPage() {
  ...
  // ── Card grid: entrance + parallax + magnetic hover ──────────────────────
  useEffect(() => {
    ...
    // Magnetic hover — separate from GSAP context so we can manage listeners manually
    const cards = Array.from(grid.querySelectorAll<HTMLElement>('.case-card-wrapper'))
    const cleanups: (() => void)[] = []
    ...
    cards.forEach((card) => {
      ...
      const onEnter = (e: MouseEvent) => {
        ...
        gsap.to(card, { x: dx * 0.3, y: dy * 0.3, duration: 0.4, ease: 'power2.out', overwrite: 'auto' })
      ...
      const onMove = (e: MouseEvent) => {
        ...
        const dx = e.clientX - (rect.left + rect.width / 2)
        const dy = e.clientY - (rect.top + rect.height / 2)
        gsap.to(card, { x: dx * 0.3, y: dy * 0.3, duration: 0.3, ease: 'power2.out', overwrite: 'auto' })
      }

      const onLeave = () => {
        gsap.to(card, { x: 0, y: 0, duration: 0.6, ease: 'elastic.out(1, 0.4)', overwrite: 'auto' })
    ...
    return () => {
      ...
      ScrollTrigger.getAll().forEach((t) => {
        if (cards.some((c) => t.vars.trigger === c)) t.kill()
      })
